...
public class CSVReader_Lv3//MonoBehaviourは継承しない
{
    ...
    static void CsvReader_Lv3()
    {
        csvFile = Resources.Load("QuizLevel3") as TextAsset;//指定したファイルをTextAssetとして読み込み(ファイル名の.csvは不要なことに注意)　最初の行（タイトル部分）も読み込まれるのでそこは使用しない
    ...
    public void Init()
    {
        CsvReader_Lv3();//QuizDataへ情報を一時格納
        //各変数へデータを格納
