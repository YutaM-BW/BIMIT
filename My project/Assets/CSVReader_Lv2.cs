...
public class CSVReader_Lv2//MonoBehaviourは継承しない
{
    ...
    static void CsvReader_Lv2()
    {
        csvFile = Resources.Load("QuizLevel2") as TextAsset;//指定したファイルをTextAssetとして読み込み(ファイル名の.csvは不要なことに注意)　最初の行（タイトル部分）も読み込まれるのでそこは使用しない
    ...
    public void Init()
    {
        CsvReader_Lv2();//QuizDataへ情報を一時格納
        //各変数へデータを格納
