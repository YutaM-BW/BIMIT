...
public class CSVReader_Lv4//MonoBehaviourは継承しない
{
    ...
    static void CsvReader_Lv4()
    {
        csvFile = Resources.Load("QuizLevel4") as TextAsset;//指定したファイルをTextAssetとして読み込み(ファイル名の.csvは不要なことに注意)　最初の行（タイトル部分）も読み込まれるのでそこは使用しない
    ...
    public void Init()
    {
        CsvReader_Lv4();//QuizDataへ情報を一時格納
        //各変数へデータを格納
